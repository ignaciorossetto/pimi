...
export function AddressChangeReviewActions({
  requestId,
}: {
  requestId: string;
}) {
  ...
  async function decidir(decision: "aprobado" | "rechazado", notasEnviar?: string) {
    ...
    const res = await fetch("/api/admin/address-change-requests/decidir", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ requestId, decision, notas: notasEnviar }),
    });
